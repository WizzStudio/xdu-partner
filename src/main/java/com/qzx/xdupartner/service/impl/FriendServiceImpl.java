...
private StringRedisTemplate stringRedisTemplate;
    ...
    public boolean judgeIfFriend(Long otherId) {
            String isFriendKey =
                    stringRedisTemplate.opsForValue().getAndExpire("" + UserHolder.getUserId() + RedisConstant.IS_FRIEND + otherId, RedisConstant.IS_FRIEND_TTL, TimeUnit.MINUTES);
            if ("1".equals(isFriendKey)) {
                return true;
            }
            Integer count =
                    query().eq("user_id", UserHolder.getUserId()).eq("friend_id", otherId).or(queryWrapper -> queryWrapper
                            .eq("user_id", otherId).eq("friend_id", UserHolder.getUserId())).count();
